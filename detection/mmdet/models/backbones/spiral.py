...
#        """manually add bn layers, used to batchnorm the output of each stage for detection and segmentation, not activated"""        
#        self.output_norm = nn.ModuleList()
#        for i in range(4):
#            # input: b,c,h,w
#            # output: b,c,h,w
#            bn_layer = m = nn.BatchNorm2d(config.embed_dims[i], affine=True)    # affine=True => bias trainable
#            self.output_norm.append(bn_layer)
#        """ends"""

#        """manually add ln layers, used to layernorm the output of each stage for detection and segmentation, activated"""        
#        self.output_norm = nn.ModuleList()
#        for i in range(4):
#            # input: b,c,h,w
#            # the permute to b,h,w,c, then permute back to b,c,h,w
#            # output: b,c,h,w
#            ln_layer = m = nn.LayerNorm(config.embed_dims[i])
#            self.output_norm.append(ln_layer)
#        """ends"""
...
#            print(f"self.norm: {self.norm}")
    ...
    def init_weights(self):
        print(f"in the init weights")
        print(f"init_cfg: {self.init_cfg}")
        ...
        if self.init_cfg is None:
            ...
            if 'state_dict' in checkpoint:
                ...
                print(f"in the model category")
                state_dict = checkpoint['model']
                print(f"succefully load the ckpt")
            else:
...
#                x = block(x)
    ...
    def forward(self, img):   # b,3,h,w
        ...
        """object detection"""
        if True:
#            """manually add bn, not activated"""
#            for i, bn_blk in enumerate(self.output_norm):
#                x[i] = bn_blk(x[i])
#            """ends"""

#            """manually add ln, activated"""
#            for i, ln_blk in enumerate(self.output_norm):
#                x[i] = x[i].permute(0,2,3,1)    # b,c,h,w => b,h,w,c
#                x[i] = ln_blk(x[i])    # b,h,w,c
#                x[i] = x[i].permute(0,3,1,2).contiguous()    # b,h,w,c => b,c,h,w
#            """ends"""
